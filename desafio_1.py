...
class Conta:
    ...
    def sacar(self, valor):
        ...
        if excedeu_saldo:
            print("Operação inválida. Saldo não disponível")
        elif valor > 0:
            self.saldo -= valor
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        ...
        if excedeu_limite:
            print("Falha na operação, limite excedido!")
...
def exibir_extrato(clientes):
    ...
    extrato = ""
    if not transacoes:
        extrato = "Não foram relizadas movimentações nesta conta"
    else:
